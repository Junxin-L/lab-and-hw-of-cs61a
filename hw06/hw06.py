...
class VendingMachine:
    ...
    def vend(self):
        ...
        if self.stock == 0:
            ...
            self.stock -= 1
            self.balance = 0
            return f'Here is your {self.obj} and ${change} change.'
        else:
            self.stock -= 1
            self.balance = 0
            return f'Here is your {self.obj}.'
    
    def restock(self, stock):
        self.stock += stock
        return f'Current {self.obj} stock: {self.stock}'
...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        year_pass = Mint.present_year - self.year - 50
        if year_pass > 0:
...
def store_digits(n):
    ...
    if div == 0:
        ...
        #return Link(res, store_digits(div))注意顺序
        head = store_digits(div)
        tail = head
        #l.rest = Link(res) 应该是要找到最后一个rest才对
        while tail.rest != Link.empty:
            tail = tail.rest
        tail.rest = Link(res)
        return head
...
def two_list(vals, amounts):
    ...
    for val in vals:
        for _ in range(amounts[i]):
            ...
            t = t.rest
        i += 1
    return l.rest  #等于把第一个元素多做了一次，在这里圆回来      
