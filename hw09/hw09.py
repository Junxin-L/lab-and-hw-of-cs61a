...
def roman_numerals(text):
    ...
    []
    """
    return re.findall(r'[IVXLCDM]*[IV]+', text)
...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search(r'((CS)|(cs))\s?[0-9]+[ABCabc]?', post))
...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r'[0-5]?[0-9]:[0-5][0-9][AP]?M?', text)
...
def area_codes(text):
    ...
    []
    """
    return re.findall(r'\(?([0-9]{3})\)?\s?[0-9]{3}\s?[0-9]{4}\b', text)
    #如果限定{4}的话后面那个还是会match到,所以我觉得{}一定要和\b一起用
...
def most_common_code(text):
    ...
    most = lst[0]
    for code in lst:
        if lst.count(most) < lst.count(code):
            most = code
    return most
