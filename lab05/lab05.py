...
def closer_city(lat, lon, city_a, city_b):
    ...
    dist_a = distance(sample_city, city_a)
    dist_b = distance(sample_city, city_b)
    if dist_a > dist_b:
...
def berry_finder(t):
    ...
    >>> berry_finder(scrat)
    True
    >>> sproul = tree('roots', [tree('branch1', [tree('berry'), tree('berry')]), tree('branch2')])
    >>> berry_finder(sproul)
    True
    ...
    for branch in branches(t):
        is_berry = is_berry + berry_finder(branch)
    return bool(is_berry)"""
    if label(t) == 'berry':
        return True
    for branch in branches(t):
...
def sprout_leaves(t, leaves):
    ...
    if is_leaf(t):
        ...
        return tree(label(t),
                    [sprout_leaves(j, leaves) for j in branches(t)])
